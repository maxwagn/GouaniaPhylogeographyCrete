# CretePopGenomics

Code and analysis pipelines accompanying the manuscript:

"ADD PREPRINT ONCE UPLOADED"

## Repository structure

- `scripts/` : custom analysis scripts
- `notebooks/` : Jupyter notebooks used for analyses and visualization
- `snakemake_pipelines/` : Snakemake workflows

## Data availability

Raw sequencing data are available from ENA (accession numbers mentioned in the publicaton).

Intermediate and processed data for local analysis are not included in this repository but are available under:
https://doi.org/10.6084/m9.figshare.32686272

## Reproducibility

Analyses were performed using Snakemake and Python (Jupyter Notebooks). Software versions are documented in the corresponding workflow and environment files.

## Contact

Maximilian Wagner
